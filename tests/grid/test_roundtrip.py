                ...
                entries.append(
                    {
                        ...
                        "inventory_list": _obj_nested_signature(
                            nested_map.get("inventory_list", [])
                        ),
                        "status_list": _obj_nested_signature(
                            nested_map.get("status_list", [])
                        ),
                    }
                )
            entries.sort(
...
def _stable_sig(value: Any) -> Any:
    ...
    if isinstance(value, AbstractSet):
        return tuple(sorted(_stable_sig(v) for v in value))
    if isinstance(value, Sequence) and (not isinstance(value, (str, bytes))):
        return tuple((_stable_sig(v) for v in value))
    return value
