...
def test_agent_pushes_box_successfully() -> None:
    ...
    check_positions(state, {agent_id: Position(1, 0), box_ids[0]: Position(2, 0)})
...
def test_push_box_out_of_bounds() -> None:
    ...
    state = step(state, Action.RIGHT, agent_id=agent_id)
    check_positions(state, {agent_id: Position(3, 0), box_ids[0]: Position(4, 0)})
...
def test_push_box_left_right_up_down() -> None:
    for action, agent_pos, box_pos, dest_pos in [
        ...
        check_positions(
            state, {agent_id: Position(*box_pos), box_ids[0]: Position(*dest_pos)}
        )
...
def test_push_box_on_narrow_grid_edge() -> None:
    ...
    state = step(state, Action.DOWN, agent_id=agent_id)
    check_positions(state, {agent_id: Position(0, 0), box_ids[0]: Position(0, 1)})
...
def test_push_no_pushable_at_destination() -> None:
    ...
    state = step(state, Action.RIGHT, agent_id=agent_id)
    check_positions(state, {agent_id: Position(1, 0)})
...
def test_push_box_blocked_by_agent() -> None:
    ...
    state = replace(
        state,
        agent={**state.agent, agent2_id: Agent()},
        collidable={**state.collidable, agent2_id: Collidable()},
        position={**state.position, agent2_id: Position(2, 0)},
        inventory={**state.inventory, agent2_id: Inventory(set())},
    )
    state = step(state, Action.RIGHT, agent_id=agent_id)
    check_positions(
        state,
...
def test_push_box_missing_position_component() -> None:
    ...
    state = replace(
        state,
        pushable={**state.pushable, missing_box_id: Pushable()},
        collidable={**state.collidable, missing_box_id: Collidable()},
        appearance={**state.appearance, missing_box_id: Appearance(name="box")},
    )
    state = step(state, Action.RIGHT, agent_id=agent_id)
...
def test_push_after_agent_moves_multiple_times() -> None:
    ...
    state = step(state, Action.LEFT, agent_id=agent_id)
    state = step(state, Action.RIGHT, agent_id=agent_id)
    assert state.position[agent_id] == Position(1, 0)
    assert state.position[box_ids[0]] == Position(2, 0)
